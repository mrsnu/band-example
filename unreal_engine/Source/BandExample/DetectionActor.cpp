...
void ADetectionActor::OnEndRequest(int32 JobId, EBandStatus Status)
{
	...
	FString status = UEnum::GetValueAsString(Status);
	UE_LOG(LogTemp, Warning, TEXT("JobId: %d, Status: %s"), JobId, *status);
	if (Status == EBandStatus::Ok)
	{
		UBandBlueprintLibrary::GetOutputs(
			JobId, DetectorOutputTensors);
		...
		UE_LOG(LogTemp, Warning, TEXT("BoundingBoxes: %d"),
		       BoundingBoxes.Num());
